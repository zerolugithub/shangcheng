                ...
                <table width="1000" align="center" id="privSel">
                    <?php if(is_array($privData)): foreach($privData as $key=>$priv): ?><tr>
                            <td class="label" align="left" width="30%">
                                <input type="checkbox" name="priv_id[]" id="priv_<?php echo ($priv["id"]); ?>" class="partSel" data="<?php echo ($priv["id"]); ?>" <?php if(in_array($priv[id],$privLst)): ?>checked<?php endif; ?> value="<?php echo ($priv["id"]); ?>">
                                <label for="priv_<?php echo ($priv["id"]); ?>"><?php echo ($priv["priv_name"]); ?></label>
                            </td>
                            <td align="left" width="70%" id="check_<?php echo ($priv["id"]); ?>">
                                <?php if(is_array($priv[_child])): foreach($priv[_child] as $k=>$son): if($son[_child]): ?><input type="checkbox" name="priv_id[]" id="priv_<?php echo ($son["id"]); ?>" class="sel" <?php if(in_array($son[id],$privLst)): ?>checked<?php endif; ?> value="<?php echo ($son["id"]); ?>">
                                        <label for="priv_<?php echo ($son["id"]); ?>"><?php echo ($son["priv_name"]); ?></label>
                                        <?php if(is_array($son[_child])): foreach($son[_child] as $i=>$v): ?><input type="checkbox" name="priv_id[]" id="priv_<?php echo ($v["id"]); ?>"  class="sel" <?php if(in_array($v[id],$privLst)): ?>checked<?php endif; ?> value="<?php echo ($v["id"]); ?>">
                                            <label for="priv_<?php echo ($v["id"]); ?>"><?php echo ($v["priv_name"]); ?></label><?php endforeach; endif; ?>
                                        <?php else: ?>
                                        <input type="checkbox" name="priv_id[]" id="priv_<?php echo ($son["id"]); ?>" class="sel" <?php if(in_array($son[id],$privLst)): ?>checked<?php endif; ?> value="<?php echo ($son["id"]); ?>">
