...
<script type="text/javascript">
    $(function () {
        $('#btn').click(function () {
            ...
            xmlObj.open('post', '<?php echo U("privilege/edit");?>');
            xmlObj.send(frmData);
        })
    ...
    <form action="" method="post" name="theForm" id="theForm">
        ...
        <table width="100%" id="general-table" align="center">
            <tr>
                ...
                <td>
                    <input type='text' name='priv_name' maxlength="20" value='<?php echo ($privData["priv_name"]); ?>' size='27'/> <font
                        color="red">*</font>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <select name="pid">
                        <?php if(is_array($privTree)): foreach($privTree as $key=>$val): ?><option value="0">顶级权限</option>
                            <?php if($val[id] == $privData.pid): ?><option value="<?php echo ($val["id"]); ?>" selected><?php echo (str_repeat('&nbsp;',$val["level"]*2)); echo ($val["priv_name"]); ?></option>
                                <?php else: ?>
                                <option value="<?php echo ($val["id"]); ?>"><?php echo (str_repeat('&nbsp;',$val["level"]*2)); echo ($val["priv_name"]); ?>
            ...
            <tr>
                ...
                <td>
                    ...
                    <?php if($privData[is_show] == 1): ?>checked<?php endif; ?>
                    /> 是
                    <input type="radio" name="is_show" value="0"
                    <?php if($privData[is_show] == 0): ?>checked<?php endif; ?>
                    /> 否
                </td>
            </tr>
            <tr>
